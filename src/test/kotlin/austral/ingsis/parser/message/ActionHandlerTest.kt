...
class ActionHandlerTest {
    ...
    fun `test handleFormat`() {
        ...
        actionHandler.handleFormat(listOf(snippetId), language, config, id)

        // Verify interactions
        verify(snippetService).getSnippetById(snippetId)
        verify(snippetService).formatSnippet(snippet, language, snippetId, config, id)
    }
    ...
    fun `test handleExecute`() {
        ...
        `when`(snippetService.getSnippetById(snippetId)).thenReturn(snippet)
        `when`(snippetService.getTests(snippetId)).thenReturn(tests)

        actionHandler.handleExecute(language, ownerId, snippetId)

        verify(snippetService).getSnippetById(snippetId)
        verify(snippetService).getTests(snippetId)
        verify(snippetService).executeMultipleTests(snippet, language, tests, snippetId, ownerId)
    }
}
